...
fn main() {
    ...
    let safe_token: Address = "0x1111111111111111111111111111111111111111".parse().unwrap();
    ...
    for (name, addr_str) in test_tokens {
        ...
        match detector.detect(token, test_amount, None, None, None, None) {
            Ok(hp_result) => {
                let status = if hp_result.is_honeypot { "🚨 HONEYPOT" } else { "✅ SAFE" };
                println!(
                    "   {} | {} | Latency: {}ms",
                    name,
                    status,
    ...
    let benchmark_token: Address = "0xDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD".parse().unwrap();
    let iterations = 100;
    ...
    println!("   Throughput: {:.0} checks/sec", iterations as f64 / total_time.as_secs_f64());
    println!();
